...
async function setUser(u){currentUser=u;if(!u){isAdmin=false;hide('admin');hide('logoutBtn');show('loginBtn');return}hide('loginBtn');show('logoutBtn');const r=await sb.from('profiles').select('role').eq('id',u.id).maybeSingle();isAdmin=r.data?.role==='admin';if(isAdmin)show('admin')}
